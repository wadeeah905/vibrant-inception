...
import { MapPin, Clock, Calendar, User, Plus, ArrowLeft, Building2, Settings, Trash2, Filter, Search, ChevronDown, X, Save, QrCode, History } from 'lucide-react';
      ...
      click(e) {
        if (isCreating) {
          const newPoint = {
            ...
            name: `Point de contrôle ${points.length + 1}`
          };
          onAddPoint(newPoint);
        ...
        <div className={`${getThemeClasses(theme, 'card')} rounded-xl p-6`}>
          ...
          <div className="h-[600px]">
            <MapComponent
              ...
              onAddPoint={() => {}}
              readOnly={true}
              selectedSite={sites.find(s => s.id === selectedRound.siteId)}
              onPointUpdate={() => {}}
            />
          </div>
        </div>
