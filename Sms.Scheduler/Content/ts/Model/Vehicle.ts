...
export class Vehicle extends ResourceModel {
	...
	DisplayName: string;
	readonly type = ResourceType.Vehicle;
	...
	get Assets(): Main.Rest.Model.Lookups.Main_Asset[] {
		...
		if (this._Assets == null || !_.isEqual(this._Assets.Metadata, keys)) {
			this._Assets = new LazyOfTTMetadata(() => keys.map(key => window.Helper.Scheduler.CreateLookupProxy(Vehicle.lookups.assets, key)), keys);
		}
	...
	get Skills(): Main.Rest.Model.Lookups.Main_Skill[] {
		...
		if (this._Skills == null || !_.isEqual(this._Skills.Metadata, keys)) {
			this._Skills = new LazyOfTTMetadata(() => keys.map(key => window.Helper.Scheduler.CreateLookupProxy(Vehicle.lookups.skills, key)), keys);
		}
